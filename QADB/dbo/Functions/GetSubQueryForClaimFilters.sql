CREATE FUNCTION [dbo].[GetSubQueryForClaimFilters]
(@DateType       INT,
 @DateFrom       DATETIME,
 @DateTo         DATETIME,
...
RETURNS VARCHAR(MAX)
     BEGIN
         ...
         SET @ClaimSubQry = 'SELECT DISTINCT CD.claimid C from ClaimData AS CD  ';
                 ...
                 SET @ClaimSubQry = CASE
                                        ...
                                        THEN CASE
                                                WHEN(@ClaimSubQry NOT LIKE '%JOIN dbo.ClaimPayVarData%')
                                                 THEN  CASE
									          WHEN @ClaimField=57 --ModelID condition as contract depends on model
											THEN @ClaimSubQry+' JOIN dbo.ClaimPayVarData AS CPVA ON CPVA.ClaimID = CD.ClaimID AND CPVA.ModelID='+CONVERT(VARCHAR(20), @ModelID)
											ELSE  @ClaimSubQry+' JOIN dbo.ClaimPayVarData AS CPVA ON CPVA.ClaimID = CD.ClaimID'
											END
                                                 ELSE @ClaimSubQry
                                             END
                                        WHEN(@ClaimField = 51
                                        ...
                                        THEN CASE
                                                 ...
                                                 ELSE @ClaimSubQry
                                             END
                 --FIXED-2016-R3-S3 - Use ClaimPayVarData view for check date & check number,  no need to join with RemitData
                 --FIXED-2016-R3-S3 - Use ClaimPayVarData view for ContractId, no need to join with ContractAdjudications
                                        ELSE @ClaimSubQry
                                    END;
         ...
         WHILE(@Count > 0)
             BEGIN
                 ...
                 IF(@ClaimField = 3) -- Revenue Code(I)   
                     BEGIN
                         DECLARE @pad_characters VARCHAR(2)= '0000', @Pad VARCHAR(2)= '0%', @ClaimChargesRevCodeSubQry VARCHAR(MAX)= '', @RevCodeBlankCondition VARCHAR(100)= ' AND ISNULL(CCD.RevCode, '''') <> ''''';
                         IF @DateFlag = 0
                             BEGIN
                                 SET @ClaimSubQry = @ClaimSubQry+' WHERE ';
                                 SET @DateFlag = 1;
                 ...
                 IF(@ClaimField = 4) -- HCPCS/RATE/HIPPS   
                     BEGIN
                         DECLARE @ClaimChargesHCPCSCodeSubQry VARCHAR(MAX)= '', @HCPCSCodeBlankCondition VARCHAR(100)= ' AND ISNULL(CCD.HCPCSCode, '''') <> ''''';
                         IF @DateFlag = 0
                             BEGIN
                                 SET @ClaimSubQry = @ClaimSubQry+' WHERE ';
                                 SET @DateFlag = 1;
                 ...
                 IF(@ClaimField = 9) -- Place Of Service
                     BEGIN
                         DECLARE @ClaimChargesPOSSubQry VARCHAR(MAX)= '', @POSBlankCondition VARCHAR(100)= ' AND ISNULL(CCD.PlaceOfService, '''') <> ''''';
                         IF @DateFlag = 0
                             BEGIN
                                 SET @ClaimSubQry = @ClaimSubQry+' WHERE ';
                                 SET @DateFlag = 1;
                 ...
                 IF(@ClaimField = -99)  -- Job RequestName  
                     BEGIN
                         ...
                         SET @ClaimSubQry = @ClaimSubQry+' EXISTS  ( '+[dbo].[GetRequestNameFilterQuery](@OperatorValue, @Values,@ModelID)
                 ...
                 IF(@ClaimField = 57) -- Adjudicated Contract Name
					...
					IF @OperatorValue='<>'
					SET @ClaimSubQry =REPLACE(@ClaimSubQry,''' ))',''') OR CPVA.ContractID IS NULL )') --To Pick Missing Contracts Claims when not equal to condition
                         SET @Flag = 1;
                     END;
                 IF(@Count > 1
                    AND @FLAG = 1)
